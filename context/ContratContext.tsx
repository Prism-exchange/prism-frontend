...
import artifactPairNewOrder from '../artifacts/contracts/PairNewOrder.sol/PairNewOrder.json'
...
import { PrismQueryPairsQuery, PrismQueryPairsQueryVariables } from '../.graphclient'
const lengthToFind = 1000
  ...
  const loadOrderBook = async () => {
    ...
    try {
      ...
      const findOrder = async (side: number) => {
        ...
        let lengthToFind = 100
        let startIndex = toWei(0)
        let dataOrder: [PairNewOrder.OrderStructOutput[], ethers.BigNumber] = [
          [],
          toWei(0),
        ]
        let listTempOrder: PairNewOrder.OrderStructOutput[] = []
        while (true) {
        ...
        allList.map(async (tickFeeID) => {
          ...
          if (
            ...
            const [
              ...
              contractFeeController.rewardRate(
                ContractPairOrderAddress,
                0,
                tickFeeID
              ),
              contractFeeController.finishAt(
                ContractPairOrderAddress,
                0,
                tickFeeID
              ),
              contractFeeController.totalSupply(
                ContractPairOrderAddress,
                0,
                tickFeeID
              ),
            ...
            const structOrder: OrderFee = {
              ...
              upperPrice: toEtherFloatingPoint(
                resultTickFeeByID.upperTickPrice,2
              ),
              lowerPrice: toEtherFloatingPoint(
                resultTickFeeByID.lowerTickPrice,2
              ),
              aprBuy: getARP(
                Number(toEther(rewardRateBuy)),
                Number(toEther(totalSupplyBuy)),
                Number(price),
                1,
                finishAtBuy.toNumber()
              ),
              aprSell: getARP(
                Number(toEther(rewardRateSell)),
                Number(toEther(totalSupplySell)),
                1,
                Number(price),
                finishAtSell.toNumber()
              ),
            }
    ...
    try {
      ...
      listOrderHistoryMarketOrder.map(async (item, index) => {
        const struct: TypeOrderMarket = {
          isBuy: item.isBuy ,
          amount:  item.isBuy == 0 ? (Number(toEther(item.amount))/ Number(toEther(item.price)) ).toFixed(6) : toEtherFloatingPoint(item.amount,6),
    ...
    try {
      ...
      if (resultFee[0]) {
        ...
        const [
          ...
          contractFeeController.rewardRate(pair, 0, resultFee[1]),
          contractFeeController.finishAt(pair, 0, resultFee[1]),
          contractFeeController.totalSupply(pair, 0, resultFee[1]),
          contractFeeController.earned(accounts, pair, 1, resultFee[1]),
          contractFeeController.rewardRate(pair, 1, resultFee[1]),
          contractFeeController.finishAt(pair, 1, resultFee[1]),
          contractFeeController.totalSupply(pair, 1, resultFee[1]),
        ])

        resultFeeData = {
          earnedBuy: toEtherFloatingPoint(earnedBuy, 6),
          rewardRateBuy: Number(toEther(rewardRateBuy)),
          finishAtBuy: finishAtBuy.toNumber(),
          totalSupplyBuy: Number(toEther(totalSupplyBuy)),
          earnedSell: toEtherFloatingPoint(earnedSell, 6),
          rewardRateSell: Number(toEther(rewardRateSell)),
          finishAtSell: finishAtSell.toNumber(),
          totalSupplySell: Number(toEther(totalSupplySell)),
        }
        price = toEtherandFixFloatingPoint(await pairOrder.price())
      }
