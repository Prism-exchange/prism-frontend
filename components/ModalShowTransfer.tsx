...
function ModalShowTransfer({
  ...
  const { sendTxTransfer, triggerWallet, loadBalanceSpotToken } =
  ...
  useEffect(() => {
    const loadBalanceAndAllowanceToken = async () => {
      ...
      setReqBalanceOfToken(
        Number(await loadBalanceSpotToken(modalAddressToken))
      )
      setIsLoading(false)
    }
   ...
   rounded-md scrollbar-hide  bgMain border-[1px] border-gray-700 px-8 py-6 outline-none"
      >
        <div className="flex flex-col gap-4 outline-none">
          ...
          <div>
            <h1 className="font-semibold mb-2">To Wallet</h1>
            <div
              className="text-white bg-gray-900 font-semibold flex flex-col text-base px-3  border-[1px] border-gray-900 
            ...
            >
              <div className="flex p-2">
                <input
                  ...
                  className=" text-sm w-full py-2 pr-2 text-left  bg-transparent outline-none  text-white "
                />
              </div>
            </div>
          </div>
          ...
          <button
            ...
            onClick={() => {
              notificationToast(
                sendTxTransfer(amountInput, modalAddressToken, addressInput)
              )
            }}
...
export default ModalShowTransfer
