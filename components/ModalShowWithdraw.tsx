...
function ModalShowWithdraw({
  ...
  const { sendTxWithdraw, triggerWallet, loadBalanceSpotToken } =
   ...
   rounded-md scrollbar-hide  bgMain border-[1px] border-gray-700 px-8 py-6 outline-none"
      >
        <div className="flex flex-col gap-4 outline-none">
          <h1 className="font-semibold text-xl textShadowWhite">Withdraw</h1>
          <button
            onClick={handleCLose}
          ...
          <div>
            <div className="flex flex-col font-semibold bg-gray-900  rounded-lg py-2 px-4 ">
              <h1>No Minimum/Max Withdraw</h1>
            </div>
          </div>
          <button
            disabled={isLoading}
            onClick={() => {
              notificationToast(sendTxWithdraw(amountInput, modalAddressToken))
            }}
...
export default ModalShowWithdraw
